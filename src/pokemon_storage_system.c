...
EWRAM_DATA static struct PokemonStorageSystemData *ssMonSummaryScreen = NULL;
...
static void FreesMonSummaryScreen(void);
...
static void VblankCb_PSS(void)
{
    ...
    sub_80D2AA4();
    TransferPlttBuffer();
    SetGpuReg(REG_OFFSET_BG2HOFS, ssMonSummaryScreen->bg2_X);
}
...
static void Cb2_EnterPSS(u8 boxOption)
{
    ...
    ssMonSummaryScreen = Alloc(sizeof(*ssMonSummaryScreen));
    if (ssMonSummaryScreen == NULL)
    {
        SetMainCallback2(Cb2_ExitPSS);
    }
    else
    {
        ssMonSummaryScreen->boxOption = boxOption;
        ssMonSummaryScreen->isReshowingPSS = FALSE;
        gUnknown_02039D12 = 0;
        ssMonSummaryScreen->state = 0;
        ssMonSummaryScreen->taskId = CreateTask(Cb_InitPSS, 3);
...
static void Cb2_ReturnToPSS(void)
{
    ...
    {
        ssMonSummaryScreen->boxOption = sCurrentBoxOption;
        ssMonSummaryScreen->isReshowingPSS = TRUE;
        ssMonSummaryScreen->state = 0;
        ssMonSummaryScreen->taskId = CreateTask(Cb_InitPSS, 3);
        SetMainCallback2(Cb2_PSS);
    }
...
static void sub_80C7E98(void)
{
    ...
    sub_80D2A90(&ssMonSummaryScreen->unk_0020, ssMonSummaryScreen->unk_0028, 8);
    gKeyRepeatStartDelay = 20;
    clear_scheduled_bg_copies_to_vram();
    sub_80D259C(3);
    sub_80D2644(0, 1, gUnknown_0857239C, 8, 4);
    sub_80D2770(0, 1, 0);
    ssMonSummaryScreen->unk_02C7 = 0;
}

static void sub_80C7F1C(void)
{
    sub_80CDC0C();
    sInPartyMenu = (ssMonSummaryScreen->boxOption == BOX_OPTION_DEPOSIT);
    gUnknown_02039D0E = 0;
}

static void sub_80C7F4C(void)
{
    if (ssMonSummaryScreen->boxOption == BOX_OPTION_MOVE_ITEMS)
    {
...
{
    gTasks[ssMonSummaryScreen->taskId].func = newFunc;
    ssMonSummaryScreen->state = 0;
}

static void Cb_InitPSS(u8 taskId)
{
    switch (ssMonSummaryScreen->state)
    {
    case 0:
        SetVBlankCallback(NULL);
        SetGpuReg(REG_OFFSET_DISPCNT, 0);
        sub_80C7E98();
        if (ssMonSummaryScreen->isReshowingPSS)
        {
    ...
    case 9:
        ...
        if (ssMonSummaryScreen->boxOption != BOX_OPTION_MOVE_ITEMS)
        {
            ssMonSummaryScreen->field_DA4.baseTileTag = TAG_TILE_D;
            ssMonSummaryScreen->field_DA4.basePaletteTag = TAG_PAL_DACE;
            sub_811F90C(&ssMonSummaryScreen->field_DA4);
            sub_811FA90();
        }
        else
        {
...
static void Cb_MainPSS(u8 taskId)
{
    ...
    case 0:
        ...
        case 1:
            PlaySE(SE_SELECT);
            ssMonSummaryScreen->state = 1;
            break;
        case 5:
            if (ssMonSummaryScreen->boxOption != BOX_OPTION_MOVE_MONS && ssMonSummaryScreen->boxOption != BOX_OPTION_MOVE_ITEMS)
            {
                PrintStorageActionText(PC_TEXT_WHICH_ONE_WILL_TAKE);
                ssMonSummaryScreen->state = 3;
            }
            else
            {
                sub_80CDC0C();
            ...
            break;
        case 6:
            if (ssMonSummaryScreen->boxOption == BOX_OPTION_MOVE_MONS)
            {
                if (IsMonBeingMoved() && ItemIsMail(ssMonSummaryScreen->cursorMonItem))
                    ssMonSummaryScreen->state = 5;
                else
                    SetPSSCallback(Cb_HidePartyPokemon);
            }
            else if (ssMonSummaryScreen->boxOption == BOX_OPTION_MOVE_ITEMS)
            {
                SetPSSCallback(Cb_HidePartyPokemon);
            }
            break;
        ...
        case 9:
            PlaySE(SE_SELECT);
            ssMonSummaryScreen->newCurrBoxId = StorageGetCurrentBox() + 1;
            if (ssMonSummaryScreen->newCurrBoxId >= TOTAL_BOXES_COUNT)
                ssMonSummaryScreen->newCurrBoxId = 0;
            if (ssMonSummaryScreen->boxOption != BOX_OPTION_MOVE_ITEMS)
            {
                SetUpScrollToBox(ssMonSummaryScreen->newCurrBoxId);
                ssMonSummaryScreen->state = 2;
            }
            else
            {
                sub_80CFEA8();
                ssMonSummaryScreen->state = 10;
            }
            break;
        case 10:
            PlaySE(SE_SELECT);
            ssMonSummaryScreen->newCurrBoxId = StorageGetCurrentBox() - 1;
            if (ssMonSummaryScreen->newCurrBoxId < 0)
                ssMonSummaryScreen->newCurrBoxId = TOTAL_BOXES_COUNT - 1;
            if (ssMonSummaryScreen->boxOption != BOX_OPTION_MOVE_ITEMS)
            {
                SetUpScrollToBox(ssMonSummaryScreen->newCurrBoxId);
                ssMonSummaryScreen->state = 2;
        ...
        case 11:
            if (!CanMovePartyMon())
            {
                if (ItemIsMail(ssMonSummaryScreen->cursorMonItem))
                {
                    ssMonSummaryScreen->state = 5;
                }
                else
        ...
        case 20:
            PlaySE(SE_SELECT);
            sub_80D01D0(0);
            ssMonSummaryScreen->state = 7;
            break;
        case 22:
            sub_80D01D0(1);
            ssMonSummaryScreen->state = 8;
            break;
        case 21:
            PlaySE(SE_SELECT);
            sub_80D01D0(2);
            ssMonSummaryScreen->state = 9;
            break;
        case 23:
            sub_80D01D0(3);
            ssMonSummaryScreen->state = 7;
    ...
    case 1:
        if (!sub_80CD554())
        {
            ...
            if (ssMonSummaryScreen->setMosaic)
                BoxSetMosaic();
            ssMonSummaryScreen->state = 0;
        }
        break;
    case 2:
        if (!ScrollToBox())
        {
            SetCurrentBox(ssMonSummaryScreen->newCurrBoxId);
...
static void Cb_OnSelectedMon(u8 taskId)
{
    ...
    case 0:
        if (!sub_80CA2B8())
        {
            ...
            else if (IsActiveItemMoving() || ssMonSummaryScreen->cursorMonItem != 0)
    ...
    case 2:
        ...
        case 1:
            ...
            else if (ItemIsMail(ssMonSummaryScreen->cursorMonItem))
            {
                ssMonSummaryScreen->state = 4;
            }
            else
        ...
        case 7:
            ...
            else if (ssMonSummaryScreen->cursorMonIsEgg)
            {
                ssMonSummaryScreen->state = 5; // Cannot release an Egg.
            }
            else if (ItemIsMail(ssMonSummaryScreen->cursorMonItem))
            {
                ssMonSummaryScreen->state = 4;
...
static void Cb_DepositMenu(u8 taskId)
{
    ...
    case 0:
        ...
        sub_80C77E8(&ssMonSummaryScreen->field_1E5C, TAG_TILE_A, TAG_PAL_DAC7, 3, FALSE);
    ...
    case 2:
        ...
        if (!sub_80CE2A8())
        {
            while (1)
            {
                ...
                {
                    ssMonSummaryScreen->state = 8; // Can't release the mon.
                    break;
                }
            }
        }
...
static void Cb_ShowMarkMenu(u8 taskId)
{
    ...
    case 0:
        ...
        ssMonSummaryScreen->field_DA4.markings = ssMonSummaryScreen->cursorMonMarkings;
        sub_811FAA4(ssMonSummaryScreen->cursorMonMarkings, 0xb0, 0x10);
        ssMonSummaryScreen->state++;
        break;
    case 1:
        if (!sub_811FBA4())
        {
            sub_811FAF8();
            ClearBottomWindow();
            SetMonMarkings(ssMonSummaryScreen->field_DA4.markings);
            RefreshCursorMonData();
            SetPSSCallback(Cb_MainPSS);
        }
        break;
...
static void Cb_ItemToBag(u8 taskId)
{
    ...
    {
    case 0:
        if (!AddBagItem(ssMonSummaryScreen->cursorMonItem, 1))
        {
            PlaySE(SE_HAZURE);
...
static void Cb_CloseBoxWhileHoldingItem(u8 taskId)
{
    ...
    case 1:
        ...
        case 1:
            ...
            break;
        case 0:
            if (AddBagItem(ssMonSummaryScreen->movingItem, 1) == TRUE)
            {
                ClearBottomWindow();
                ssMonSummaryScreen->state = 3;
            }
...
static void Cb_HandleWallpapers(u8 taskId)
{
    ...
    case 1:
        ...
        break;
    case 2:
        ssMonSummaryScreen->wallpaperSetId = sub_80D00AC();
        switch (ssMonSummaryScreen->wallpaperSetId)
        {
        case -1:
            sub_80CD1A8(TRUE);
            ClearBottomWindow();
        ...
        case 18 ... 21:
            PlaySE(SE_SELECT);
            sub_80D013C();
            ssMonSummaryScreen->wallpaperSetId -= 18;
            ssMonSummaryScreen->state++;
            break;
        // New wallpaper from Walda.
        case 22:
            PlaySE(SE_SELECT);
            ssMonSummaryScreen->wallpaperId = 16;
            sub_80D013C();
            ClearBottomWindow();
            ssMonSummaryScreen->state = 6;
            break;
    ...
    case 3:
        if (!IsDma3ManagerBusyWithBgCopy())
        {
            AddWallpapersMenu(ssMonSummaryScreen->wallpaperSetId);
            PrintStorageActionText(PC_TEXT_PICK_A_WALLPAPER);
            ssMonSummaryScreen->state++;
        }
        break;
    case 4:
        ssMonSummaryScreen->wallpaperId = sub_80D00AC();
        switch (ssMonSummaryScreen->wallpaperId)
        {
        case -2:
            break;
        case -1:
        ...
        default:
            PlaySE(SE_SELECT);
            ClearBottomWindow();
            ssMonSummaryScreen->wallpaperId -= 23;
            SetWallpaperForCurrentBox(ssMonSummaryScreen->wallpaperId);
            ssMonSummaryScreen->state++;
            break;
        }
        break;
...
static void Cb_JumpBox(u8 taskId)
{
    ...
    case 0:
        ...
        break;
    case 1:
        ssMonSummaryScreen->newCurrBoxId = HandleBoxChooseSelectionInput();
        switch (ssMonSummaryScreen->newCurrBoxId)
        {
        case 200:
            break;
        default:
            ...
            sub_80C78E4();
            sub_80C7890();
            if (ssMonSummaryScreen->newCurrBoxId == 201 || ssMonSummaryScreen->newCurrBoxId == StorageGetCurrentBox())
            {
                sub_80CD1A8(TRUE);
                SetPSSCallback(Cb_MainPSS);
            }
...
static void Cb_NameBox(u8 taskId)
{
    ...
    case 1:
        ...
        {
            sWhichToReshow = 1;
            ssMonSummaryScreen->screenChangeType = SCREEN_CHANGE_NAME_BOX;
            SetPSSCallback(Cb_ChangeScreen);
        }
        break;
    }
...
static void Cb_ShowMonSummary(u8 taskId)
{
    ...
    case 1:
        ...
        {
            sWhichToReshow = 0;
            ssMonSummaryScreen->screenChangeType = SCREEN_CHANGE_SUMMARY_SCREEN;
            SetPSSCallback(Cb_ChangeScreen);
        }
        break;
    }
...
static void Cb_GiveItemFromBag(u8 taskId)
{
    ...
    case 1:
        ...
        {
            sWhichToReshow = 2;
            ssMonSummaryScreen->screenChangeType = SCREEN_CHANGE_ITEM_FROM_BAG;
            SetPSSCallback(Cb_ChangeScreen);
        }
        break;
    }
...
static void Cb_OnCloseBoxPressed(u8 taskId)
{
    ...
    case 4:
        if (!sub_80F9C30())
        {
            ...
            ssMonSummaryScreen->screenChangeType = SCREEN_CHANGE_EXIT_BOX;
            SetPSSCallback(Cb_ChangeScreen);
        }
        break;
    }
...
static void Cb_ChangeScreen(u8 taskId)
{
    ...
    u8 screenChangeType = ssMonSummaryScreen->screenChangeType;

    if (ssMonSummaryScreen->boxOption == BOX_OPTION_MOVE_ITEMS && IsActiveItemMoving() == TRUE)
    ...
    default:
        FreesMonSummaryScreen();
        SetMainCallback2(Cb2_ExitPSS);
        break;
    case SCREEN_CHANGE_SUMMARY_SCREEN:
        boxMons = ssMonSummaryScreen->field_218C.box;
        monIndex = ssMonSummaryScreen->field_2187;
        maxMonIndex = ssMonSummaryScreen->field_2186;
        mode = ssMonSummaryScreen->field_2188;
        FreesMonSummaryScreen();
...
static void FreesMonSummaryScreen(void)
{
    sub_80D25F0();
    sub_80D01B8();
    FREE_AND_SET_NULL(ssMonSummaryScreen);
...
static void LoadPSSMenuGfx(void)
{
    ...
    LZ77UnCompWram(gUnknown_085722A0, ssMonSummaryScreen->field_5AC4);
    SetBgTilemapBuffer(1, ssMonSummaryScreen->field_5AC4);
    ShowBg(1);
...
static void sub_80CA154(void)
{
    ssMonSummaryScreen->field_D94 = sub_811FFB4(TAG_TILE_10, TAG_PAL_DAC8, NULL);
    ssMonSummaryScreen->field_D94->oam.priority = 1;
    ssMonSummaryScreen->field_D94->subpriority = 1;
    ssMonSummaryScreen->field_D94->pos1.x = 40;
    ssMonSummaryScreen->field_D94->pos1.y = 150;
    ssMonSummaryScreen->field_DA0 = (void*) OBJ_VRAM0 + 32 * GetSpriteTileStartByTag(TAG_TILE_10);
}
...
static void sub_80CA1C4(void)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        ssMonSummaryScreen->field_D98[i] = &gSprites[spriteId];
    }
}

static void RefreshCursorMonData(void)
{
    LoadCursorMonGfx(ssMonSummaryScreen->cursorMonSpecies, ssMonSummaryScreen->cursorMonPersonality);
    PrintCursorMonInfo();
    sub_80CA65C();
...
{
    RefreshCursorMonData();
    if (ssMonSummaryScreen->cursorMonSprite)
    {
        ssMonSummaryScreen->cursorMonSprite->oam.mosaic = TRUE;
        ssMonSummaryScreen->cursorMonSprite->data[0] = 10;
        ssMonSummaryScreen->cursorMonSprite->data[1] = 1;
        ssMonSummaryScreen->cursorMonSprite->callback = sub_80CA2D0;
        SetGpuReg(REG_OFFSET_MOSAIC, (ssMonSummaryScreen->cursorMonSprite->data[0] << 12) | (ssMonSummaryScreen->cursorMonSprite->data[0] << 8));
    }
}

static u8 sub_80CA2B8(void)
{
    return ssMonSummaryScreen->cursorMonSprite->oam.mosaic;
}
...
static void LoadCursorMonSprite(void)
{
    ...
    u8 palSlot;
    u8 spriteId;
    struct SpriteSheet sheet = {ssMonSummaryScreen->field_22C4, 0x800, TAG_TILE_2};
    struct SpritePalette palette = {ssMonSummaryScreen->field_2244, TAG_PAL_DAC6};
    struct SpriteTemplate template = sSpriteTemplate_CursorMon;

    for (i = 0; i < 0x800; i++)
        ssMonSummaryScreen->field_22C4[i] = 0;
    for (i = 0; i < 0x10; i++)
        ssMonSummaryScreen->field_2244[i] = 0;

    ssMonSummaryScreen->cursorMonSprite = NULL;
    ...
    do
    {
        ...
        ssMonSummaryScreen->cursorMonSprite = &gSprites[spriteId];
        ssMonSummaryScreen->field_223A = palSlot * 16 + 0x100;
        ssMonSummaryScreen->field_223C = (void*) OBJ_VRAM0 + tileStart * 32;
    } while (0);

    if (ssMonSummaryScreen->cursorMonSprite == NULL)
    {
...
static void LoadCursorMonGfx(u16 species, u32 pid)
{
    ...
    {
        LoadSpecialPokePic(&gMonFrontPicTable[species], ssMonSummaryScreen->field_22C4, species, pid, TRUE);
        LZ77UnCompWram(ssMonSummaryScreen->cursorMonPalette, ssMonSummaryScreen->field_2244);
        CpuCopy32(ssMonSummaryScreen->field_22C4, ssMonSummaryScreen->field_223C, 0x800);
        LoadPalette(ssMonSummaryScreen->field_2244, ssMonSummaryScreen->field_223A, 0x20);
        ssMonSummaryScreen->cursorMonSprite->invisible = FALSE;
    }
    else
    {
        ssMonSummaryScreen->cursorMonSprite->invisible = TRUE;
    }
}
...
static void PrintCursorMonInfo(void)
{
    ...
    if (ssMonSummaryScreen->boxOption != BOX_OPTION_MOVE_ITEMS)
    {
        AddTextPrinterParameterized(0, 1, ssMonSummaryScreen->cursorMonNickText, 6, 0, TEXT_SPEED_FF, NULL);
        AddTextPrinterParameterized(0, 2, ssMonSummaryScreen->cursorMonSpeciesName, 6, 15, TEXT_SPEED_FF, NULL);
        AddTextPrinterParameterized(0, 2, ssMonSummaryScreen->cursorMonGenderLvlText, 10, 29, TEXT_SPEED_FF, NULL);
        AddTextPrinterParameterized(0, 0, ssMonSummaryScreen->cursorMonItemName, 6, 43, TEXT_SPEED_FF, NULL);
    }
    else
    {
        AddTextPrinterParameterized(0, 0, ssMonSummaryScreen->cursorMonItemName, 6, 0, TEXT_SPEED_FF, NULL);
        AddTextPrinterParameterized(0, 1, ssMonSummaryScreen->cursorMonNickText, 6, 13, TEXT_SPEED_FF, NULL);
        AddTextPrinterParameterized(0, 2, ssMonSummaryScreen->cursorMonSpeciesName, 6, 28, TEXT_SPEED_FF, NULL);
        AddTextPrinterParameterized(0, 2, ssMonSummaryScreen->cursorMonGenderLvlText, 10, 42, TEXT_SPEED_FF, NULL);
    }

    CopyWindowToVram(0, 2);
    if (ssMonSummaryScreen->cursorMonSpecies != SPECIES_NONE)
    {
        sub_8120084(ssMonSummaryScreen->cursorMonMarkings, ssMonSummaryScreen->field_DA0);
        ssMonSummaryScreen->field_D94->invisible = FALSE;
    }
    else
    {
        ssMonSummaryScreen->field_D94->invisible = TRUE;
    }
}
...
static void sub_80CA65C(void)
{
    ...
    if (ssMonSummaryScreen->cursorMonSpecies != SPECIES_NONE)
    {
        ...
        for (i = 0; i < 2; i++)
            StartSpriteAnimIfDifferent(ssMonSummaryScreen->field_D98[i], i * 2 + 1);
    }
    else
    {
        sub_80D27AC(0, 0, 2, 8, 2);
        for (i = 0; i < 2; i++)
            StartSpriteAnim(ssMonSummaryScreen->field_D98[i], i * 2);
    }
...
static void sub_80CA704(void)
{
    LZ77UnCompWram(gUnknown_08DD36C8, ssMonSummaryScreen->field_B0);
    LoadPalette(gPSSMenu_Pal, 0x10, 0x20);
    sub_80D2644(1, 1, ssMonSummaryScreen->field_B0, 12, 22);
...
{
    ssMonSummaryScreen->field_2C0 = 20;
    ssMonSummaryScreen->field_2C2 = 2;
    ssMonSummaryScreen->field_2C5 = 0;
    CreatePartyMonsSprites(FALSE);
}

static bool8 ShowPartyMenu(void)
{
    if (ssMonSummaryScreen->field_2C5 == 20)
        return FALSE;

    ssMonSummaryScreen->field_2C0--;
    ssMonSummaryScreen->field_2C2++;
    sub_80D27F4(1, 3, 1);
    sub_80D2918(1);
    schedule_bg_copy_tilemap_to_vram(1);
    sub_80CBAF0(8);
...
static void SetUpHidePartyMenu(void)
{
    ssMonSummaryScreen->field_2C0 = 0;
    ssMonSummaryScreen->field_2C2 = 22;
    ssMonSummaryScreen->field_2C5 = 0;
    if (ssMonSummaryScreen->boxOption == BOX_OPTION_MOVE_ITEMS)
        sub_80D11CC();
}
...
static bool8 HidePartyMenu(void)
{
    if (ssMonSummaryScreen->field_2C5 != 20)
    {
        ...
        sub_80D27F4(1, 3, -1);
        sub_80D2918(1);
        FillBgTilemapBufferRect_Palette0(1, 0x100, 10, ssMonSummaryScreen->field_2C2, 12, 1);
        sub_80CBAF0(-8);
        if (++ssMonSummaryScreen->field_2C5 != 20)
        {
            schedule_bg_copy_tilemap_to_vram(1);
...
static void sub_80CA9C0(void)
{
    ssMonSummaryScreen->unk_02C7 = 1;
    ssMonSummaryScreen->unk_02C8 = 30;
    ssMonSummaryScreen->unk_02C9 = TRUE;
}

static void sub_80CA9EC(void)
{
    if (ssMonSummaryScreen->unk_02C7)
    {
        ssMonSummaryScreen->unk_02C7 = 0;
        sub_80CA984(TRUE);
    }
...
static void sub_80CAA14(void)
{
    if (ssMonSummaryScreen->unk_02C7 && ++ssMonSummaryScreen->unk_02C8 > 30)
    {
        ssMonSummaryScreen->unk_02C8 = 0;
        ssMonSummaryScreen->unk_02C9 = (ssMonSummaryScreen->unk_02C9 == FALSE);
        sub_80CA984(ssMonSummaryScreen->unk_02C9);
    }
}
...
static void sub_80CAAA8(u8 arg0, bool8 isPartyMon)
{
    ...
    for (i = 0; i < 3; i++)
    {
        for (j = 0; j < 4; j++)
        {
            ssMonSummaryScreen->field_B0[index + j] = data[j];
        }
        data += 4;
        index += 12;
    }
...
static void SetUpDoShowPartyMenu(void)
{
    ssMonSummaryScreen->showPartyMenuState = 0;
    PlaySE(SE_WIN_OPEN);
    SetUpShowPartyMenu();
}
...
static bool8 DoShowPartyMenu(void)
{
    switch (ssMonSummaryScreen->showPartyMenuState)
    {
    case 0:
        if (!ShowPartyMenu())
        {
            sub_80CDBA0();
            ssMonSummaryScreen->showPartyMenuState++;
        }
        break;
    case 1:
        if (!sub_80CD554())
...
static void PrintStorageActionText(u8 id)
{
    ...
    case PC_TEXT_FMT_MON_NAME_3:
        DynamicPlaceholderTextUtil_SetPlaceholderPtr(0, ssMonSummaryScreen->cursorMonNick);
        break;
    case PC_TEXT_FMT_MON_NAME_4:
    case PC_TEXT_FMT_MON_NAME_5:
    case PC_TEXT_FMT_MON_NAME_6:
        DynamicPlaceholderTextUtil_SetPlaceholderPtr(0, ssMonSummaryScreen->field_21E0);
        break;
    case PC_TEXT_FMT_ITEM_NAME:
        if (IsActiveItemMoving())
            txtPtr = StringCopy(ssMonSummaryScreen->itemName, GetMovingItemName());
        else
            txtPtr = StringCopy(ssMonSummaryScreen->itemName, ssMonSummaryScreen->cursorMonItemName);

        while (*(txtPtr - 1) == CHAR_SPACE)
            txtPtr--;

        *txtPtr = EOS;
        DynamicPlaceholderTextUtil_SetPlaceholderPtr(0, ssMonSummaryScreen->itemName);
        break;
    }

    DynamicPlaceholderTextUtil_ExpandPlaceholders(ssMonSummaryScreen->field_2190, gPCStorageActionTexts[id].text);
    FillWindowPixelBuffer(1, PIXEL_FILL(1));
    AddTextPrinterParameterized(1, 1, ssMonSummaryScreen->field_2190, 0, 1, TEXT_SPEED_FF, NULL);
...
static void sub_80CAF04(void)
{
    ...
    LoadMonIconPalettes();
    for (i = 0; i < 40; i++)
        ssMonSummaryScreen->field_B08[i] = 0;
    for (i = 0; i < 40; i++)
        ssMonSummaryScreen->field_B58[i] = 0;
    for (i = 0; i < PARTY_SIZE; i++)
        ssMonSummaryScreen->partySprites[i] = NULL;
    for (i = 0; i < IN_BOX_COUNT; i++)
        ssMonSummaryScreen->boxMonsSprites[i] = NULL;

    ssMonSummaryScreen->movingMonSprite = NULL;
    ssMonSummaryScreen->field_78C = 0;
}
...
{
    u32 personality = GetMonData(&ssMonSummaryScreen->movingMon, MON_DATA_PERSONALITY);
    u16 species = GetMonData(&ssMonSummaryScreen->movingMon, MON_DATA_SPECIES2);
    u8 priority = sub_80CAFAC();

    ssMonSummaryScreen->movingMonSprite = CreateMonIconSprite(species, personality, 0, 0, priority, 7);
    ssMonSummaryScreen->movingMonSprite->callback = sub_80CC100;
}
    ...
    for (i = 0; i < IN_BOX_COLUMNS; i++)
    {
        for (j = 0; j < IN_BOX_ROWS; j++)
        {
            ...
            if (species != SPECIES_NONE)
            {
                ...
                ssMonSummaryScreen->boxMonsSprites[count] = CreateMonIconSprite(species, personality, 8 * (3 * j) + 100, 8 * (3 * i) + 44, 2, 19 - j);
            }
            else
            {
                ssMonSummaryScreen->boxMonsSprites[count] = NULL;
            }
            boxPosition++;
            count++;
        }
    ...
    if (ssMonSummaryScreen->boxOption == BOX_OPTION_MOVE_ITEMS)
    {
        for (boxPosition = 0; boxPosition < IN_BOX_COUNT; boxPosition++)
        {
            if (GetBoxMonDataAt(boxId, boxPosition, MON_DATA_HELD_ITEM) == 0)
                ssMonSummaryScreen->boxMonsSprites[boxPosition]->oam.objMode = 1;
        }
    }
}
...
static void sub_80CB140(u8 boxPosition)
{
    ...
    if (species != SPECIES_NONE)
    {
        ...
        ssMonSummaryScreen->boxMonsSprites[boxPosition] = CreateMonIconSprite(species, personality, x, y, 2, 19 - (boxPosition % IN_BOX_ROWS));
        if (ssMonSummaryScreen->boxOption == BOX_OPTION_MOVE_ITEMS)
            ssMonSummaryScreen->boxMonsSprites[boxPosition]->oam.objMode = 1;
    }
}
...
static void sub_80CB1F0(s16 arg0)
{
    ...
    for (i = 0; i < IN_BOX_COUNT; i++)
    {
        if (ssMonSummaryScreen->boxMonsSprites[i] != NULL)
        {
            ssMonSummaryScreen->boxMonsSprites[i]->data[2] = arg0;
            ssMonSummaryScreen->boxMonsSprites[i]->data[4] = 1;
            ssMonSummaryScreen->boxMonsSprites[i]->callback = sub_80CB278;
        }
    }
}
...
static void DestroyAllIconsInRow(u8 row)
{
    ...
    {
        if (ssMonSummaryScreen->boxMonsSprites[boxPosition] != NULL)
        {
            DestroyBoxMonIcon(ssMonSummaryScreen->boxMonsSprites[boxPosition]);
            ssMonSummaryScreen->boxMonsSprites[boxPosition] = NULL;
        }
...
static u8 sub_80CB2F8(u8 row, u16 times, s16 xDelta)
{
    ...
    if (ssMonSummaryScreen->boxOption != BOX_OPTION_MOVE_ITEMS)
    {
        for (i = 0; i < IN_BOX_COLUMNS; i++)
        {
            if (ssMonSummaryScreen->boxSpecies[boxPosition] != SPECIES_NONE)
            {
                ssMonSummaryScreen->boxMonsSprites[boxPosition] = CreateMonIconSprite(ssMonSummaryScreen->boxSpecies[boxPosition],
                                                                                        ssMonSummaryScreen->boxPersonalities[boxPosition],
                                                                                        x, y, 2, subpriority);
                if (ssMonSummaryScreen->boxMonsSprites[boxPosition] != NULL)
                {
                    ssMonSummaryScreen->boxMonsSprites[boxPosition]->data[1] = times;
                    ssMonSummaryScreen->boxMonsSprites[boxPosition]->data[2] = xDelta;
                    ssMonSummaryScreen->boxMonsSprites[boxPosition]->data[3] = xDest;
                    ssMonSummaryScreen->boxMonsSprites[boxPosition]->callback = sub_80CB234;
                    count++;
                }
            }
            boxPosition += IN_BOX_ROWS;
        ...
        for (i = 0; i < IN_BOX_COLUMNS; i++)
        {
            if (ssMonSummaryScreen->boxSpecies[boxPosition] != SPECIES_NONE)
            {
                ...
                if (ssMonSummaryScreen->boxMonsSprites[boxPosition] != NULL)
                {
                    ...
                    ssMonSummaryScreen->boxMonsSprites[boxPosition]->data[3] = xDest;
                    ssMonSummaryScreen->boxMonsSprites[boxPosition]->callback = sub_80CB234;
                    if (GetBoxMonDataAt(ssMonSummaryScreen->field_C5C, boxPosition, MON_DATA_HELD_ITEM) == 0)
                        ssMonSummaryScreen->boxMonsSprites[boxPosition]->oam.objMode = 1;
                    count++;
                }
            }
...
{
    ssMonSummaryScreen->field_C6A = 0;
    ssMonSummaryScreen->field_C6B = boxId;
    ssMonSummaryScreen->field_C69 = direction;
    ssMonSummaryScreen->field_C60 = 32;
    ssMonSummaryScreen->field_C64 = -(6 * direction);
    ssMonSummaryScreen->field_C66 = 0;
    SetBoxSpeciesAndPersonalities(boxId);
    if (direction > 0)
        ssMonSummaryScreen->field_C68 = 0;
    else
        ssMonSummaryScreen->field_C68 = IN_BOX_ROWS - 1;

    ssMonSummaryScreen->field_C62 = (24 * ssMonSummaryScreen->field_C68) + 100;
    sub_80CB1F0(ssMonSummaryScreen->field_C64);
}

static bool8 sub_80CB584(void)
{
    if (ssMonSummaryScreen->field_C60 != 0)
        ssMonSummaryScreen->field_C60--;

    switch (ssMonSummaryScreen->field_C6A)
    {
    case 0:
        ssMonSummaryScreen->field_C62 += ssMonSummaryScreen->field_C64;
        if (ssMonSummaryScreen->field_C62 <= 64 || ssMonSummaryScreen->field_C62 >= 252)
        {
            DestroyAllIconsInRow(ssMonSummaryScreen->field_C68);
            ssMonSummaryScreen->field_C62 += ssMonSummaryScreen->field_C69 * 24;
            ssMonSummaryScreen->field_C6A++;
        }
        break;
    case 1:
        ssMonSummaryScreen->field_C62 += ssMonSummaryScreen->field_C64;
        ssMonSummaryScreen->field_C66 += sub_80CB2F8(ssMonSummaryScreen->field_C68, ssMonSummaryScreen->field_C60, ssMonSummaryScreen->field_C64);
        if ((ssMonSummaryScreen->field_C69 > 0 && ssMonSummaryScreen->field_C68 == IN_BOX_ROWS - 1)
            || (ssMonSummaryScreen->field_C69 < 0 && ssMonSummaryScreen->field_C68 == 0))
        {
            ssMonSummaryScreen->field_C6A++;
        }
        else
        {
            ssMonSummaryScreen->field_C68 += ssMonSummaryScreen->field_C69;
            ssMonSummaryScreen->field_C6A = 0;
        }
        break;
    case 2:
        if (ssMonSummaryScreen->field_C66 == 0)
        {
            ssMonSummaryScreen->field_C60++;
            return FALSE;
        }
...
static void SetBoxSpeciesAndPersonalities(u8 boxId)
{
    ...
    for (i = 0; i < IN_BOX_COLUMNS; i++)
    {
        for (j = 0; j < IN_BOX_ROWS; j++)
        {
            ssMonSummaryScreen->boxSpecies[boxPosition] = GetBoxMonDataAt(boxId, boxPosition, MON_DATA_SPECIES2);
            if (ssMonSummaryScreen->boxSpecies[boxPosition] != SPECIES_NONE)
                ssMonSummaryScreen->boxPersonalities[boxPosition] = GetBoxMonDataAt(boxId, boxPosition, MON_DATA_PERSONALITY);
            boxPosition++;
        }
    }

    ssMonSummaryScreen->field_C5C = boxId;
}
...
static void SetBoxMonIconObjMode(u8 boxPosition, u8 objMode)
{
    if (ssMonSummaryScreen->boxMonsSprites[boxPosition] != NULL)
    {
        ssMonSummaryScreen->boxMonsSprites[boxPosition]->oam.objMode = objMode;
    }
}
...
static void CreatePartyMonsSprites(bool8 arg0)
{
    ...
    ssMonSummaryScreen->partySprites[0] = CreateMonIconSprite(species, personality, 104, 64, 1, 12);
    count = 1;
    for (i = 1; i < PARTY_SIZE; i++)
    {
        ...
        if (species != SPECIES_NONE)
        {
            ...
            ssMonSummaryScreen->partySprites[i] = CreateMonIconSprite(species, personality, 152,  8 * (3 * (i - 1)) + 16, 1, 12);
            count++;
        }
        else
        {
    ...
    if (!arg0)
    {
        for (i = 0; i < count; i++)
        {
            ssMonSummaryScreen->partySprites[i]->pos1.y -= 160;
            ssMonSummaryScreen->partySprites[i]->invisible = TRUE;
        }
    }
    ...
    if (ssMonSummaryScreen->boxOption == BOX_OPTION_MOVE_ITEMS)
    {
        for (i = 0; i < PARTY_SIZE; i++)
        {
            if (ssMonSummaryScreen->partySprites[i] != NULL && GetMonData(&gPlayerParty[i], MON_DATA_HELD_ITEM) == 0)
                ssMonSummaryScreen->partySprites[i]->oam.objMode = 1;
        }
    }
}
...
static void sub_80CB950(void)
{
    ...
    ssMonSummaryScreen->field_C5E = 0;
    for (i = 0, count = 0; i < PARTY_SIZE; i++)
    {
        if (ssMonSummaryScreen->partySprites[i] != NULL)
        {
            if (i != count)
            {
                sub_80CB9D0(ssMonSummaryScreen->partySprites[i], count);
                ssMonSummaryScreen->partySprites[i] = NULL;
                ssMonSummaryScreen->field_C5E++;
            }
            count++;
        }
    }
...
static u8 sub_80CB9BC(void)
{
    return ssMonSummaryScreen->field_C5E;
}
...
static void sub_80CBA3C(struct Sprite *sprite)
{
    ...
    else
    {
        ...
        sprite->callback = SpriteCallbackDummy;
        ssMonSummaryScreen->partySprites[sprite->data[1]] = sprite;
        ssMonSummaryScreen->field_C5E--;
    }
}
...
{
    if (ssMonSummaryScreen->movingMonSprite != NULL)
    {
        DestroyBoxMonIcon(ssMonSummaryScreen->movingMonSprite);
        ssMonSummaryScreen->movingMonSprite = NULL;
    }
}
...
static void sub_80CBAF0(s16 yDelta)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        if (ssMonSummaryScreen->partySprites[i] != NULL)
        {
            ssMonSummaryScreen->partySprites[i]->pos1.y += yDelta;
            posY = ssMonSummaryScreen->partySprites[i]->pos1.y + ssMonSummaryScreen->partySprites[i]->pos2.y + ssMonSummaryScreen->partySprites[i]->centerToCornerVecY;
            posY += 16;
            if (posY > 192)
                ssMonSummaryScreen->partySprites[i]->invisible = TRUE;
            else
                ssMonSummaryScreen->partySprites[i]->invisible = FALSE;
        }
    }
}
...
{
    if (ssMonSummaryScreen->partySprites[partyId] != NULL)
    {
        DestroyBoxMonIcon(ssMonSummaryScreen->partySprites[partyId]);
        ssMonSummaryScreen->partySprites[partyId] = NULL;
    }
}
...
static void DestroyAllPartyMonIcons(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        if (ssMonSummaryScreen->partySprites[i] != NULL)
        {
            DestroyBoxMonIcon(ssMonSummaryScreen->partySprites[i]);
            ssMonSummaryScreen->partySprites[i] = NULL;
        }
    }
}
...
static void SetPartyMonIconObjMode(u8 partyId, u8 objMode)
{
    if (ssMonSummaryScreen->partySprites[partyId] != NULL)
    {
        ssMonSummaryScreen->partySprites[partyId]->oam.objMode = objMode;
    }
}
...
static void sub_80CBC14(u8 mode, u8 id)
{
    if (mode == MODE_PARTY)
    {
        ssMonSummaryScreen->movingMonSprite = ssMonSummaryScreen->partySprites[id];
        ssMonSummaryScreen->partySprites[id] = NULL;
    }
    else if (mode == MODE_BOX)
    {
        ssMonSummaryScreen->movingMonSprite = ssMonSummaryScreen->boxMonsSprites[id];
        ssMonSummaryScreen->boxMonsSprites[id] = NULL;
    }
    else
    {
        return;
    ...
    ssMonSummaryScreen->movingMonSprite->callback = sub_80CC100;
    ssMonSummaryScreen->movingMonSprite->oam.priority = sub_80CAFAC();
    ssMonSummaryScreen->movingMonSprite->subpriority = 7;
}
...
static void sub_80CBCAC(u8 boxId, u8 position)
{
    ...
    {
        ssMonSummaryScreen->partySprites[position] = ssMonSummaryScreen->movingMonSprite;
        ssMonSummaryScreen->partySprites[position]->oam.priority = 1;
        ssMonSummaryScreen->partySprites[position]->subpriority = 12;
    }
    else
    {
        ssMonSummaryScreen->boxMonsSprites[position] = ssMonSummaryScreen->movingMonSprite;
        ssMonSummaryScreen->boxMonsSprites[position]->oam.priority = 2;
        ssMonSummaryScreen->boxMonsSprites[position]->subpriority = 19 - (position % IN_BOX_ROWS);
    }
    ssMonSummaryScreen->movingMonSprite->callback = SpriteCallbackDummy;
    ssMonSummaryScreen->movingMonSprite = NULL;
}
...
{
    if (boxId == TOTAL_BOXES_COUNT) // party mon
        ssMonSummaryScreen->field_B00 = &ssMonSummaryScreen->partySprites[position];
    else
        ssMonSummaryScreen->field_B00 = &ssMonSummaryScreen->boxMonsSprites[position];

    ssMonSummaryScreen->movingMonSprite->callback = SpriteCallbackDummy;
    ssMonSummaryScreen->field_C5D = 0;
}

static bool8 sub_80CBDC4(void)
{
    if (ssMonSummaryScreen->field_C5D == 16)
        return FALSE;

    ssMonSummaryScreen->field_C5D++;
    if (ssMonSummaryScreen->field_C5D & 1)
    {
        (*ssMonSummaryScreen->field_B00)->pos1.y--;
        ssMonSummaryScreen->movingMonSprite->pos1.y++;
    }

    (*ssMonSummaryScreen->field_B00)->pos2.x = gSineTable[ssMonSummaryScreen->field_C5D * 8] / 16;
    ssMonSummaryScreen->movingMonSprite->pos2.x = -(gSineTable[ssMonSummaryScreen->field_C5D * 8] / 16);
    if (ssMonSummaryScreen->field_C5D == 8)
    {
        ssMonSummaryScreen->movingMonSprite->oam.priority = (*ssMonSummaryScreen->field_B00)->oam.priority;
        ssMonSummaryScreen->movingMonSprite->subpriority = (*ssMonSummaryScreen->field_B00)->subpriority;
        (*ssMonSummaryScreen->field_B00)->oam.priority = sub_80CAFAC();
        (*ssMonSummaryScreen->field_B00)->subpriority = 7;
    }

    if (ssMonSummaryScreen->field_C5D == 16)
    {
        struct Sprite *sprite = ssMonSummaryScreen->movingMonSprite;
        ssMonSummaryScreen->movingMonSprite = (*ssMonSummaryScreen->field_B00);
        *ssMonSummaryScreen->field_B00 = sprite;

        ssMonSummaryScreen->movingMonSprite->callback = sub_80CC100;
        (*ssMonSummaryScreen->field_B00)->callback = SpriteCallbackDummy;
    }
...
static void sub_80CBF14(u8 mode, u8 position)
{
    ...
    {
    case MODE_PARTY:
        ssMonSummaryScreen->field_B04 = &ssMonSummaryScreen->partySprites[position];
        break;
    case MODE_BOX:
        ssMonSummaryScreen->field_B04 = &ssMonSummaryScreen->boxMonsSprites[position];
        break;
    case MODE_2:
        ssMonSummaryScreen->field_B04 = &ssMonSummaryScreen->movingMonSprite;
        break;
    default:
        return;
    }

    if (*ssMonSummaryScreen->field_B04 != NULL)
    {
        InitSpriteAffineAnim(*ssMonSummaryScreen->field_B04);
        (*ssMonSummaryScreen->field_B04)->oam.affineMode = 1;
        (*ssMonSummaryScreen->field_B04)->affineAnims = gSpriteAffineAnimTable_857291C;
        StartSpriteAffineAnim(*ssMonSummaryScreen->field_B04, 0);
    }
}

static bool8 sub_80CBFD8(void)
{
    if (*ssMonSummaryScreen->field_B04 == NULL || (*ssMonSummaryScreen->field_B04)->invisible)
        return FALSE;

    if ((*ssMonSummaryScreen->field_B04)->affineAnimEnded)
        (*ssMonSummaryScreen->field_B04)->invisible = TRUE;
...
static void sub_80CC020(void)
{
    if (*ssMonSummaryScreen->field_B04 != NULL)
    {
        FreeOamMatrix((*ssMonSummaryScreen->field_B04)->oam.matrixNum);
        DestroyBoxMonIcon(*ssMonSummaryScreen->field_B04);
        *ssMonSummaryScreen->field_B04 = NULL;
    }
}
...
static void sub_80CC064(void)
{
    if (*ssMonSummaryScreen->field_B04 != NULL)
    {
        (*ssMonSummaryScreen->field_B04)->invisible = FALSE;
        StartSpriteAffineAnim(*ssMonSummaryScreen->field_B04, 1);
    }
}
...
static void SetMovingMonPriority(u8 priority)
{
    ssMonSummaryScreen->movingMonSprite->oam.priority = priority;
}

static void sub_80CC100(struct Sprite *sprite)
{
    sprite->pos1.x = ssMonSummaryScreen->field_CB4->pos1.x;
    sprite->pos1.y = ssMonSummaryScreen->field_CB4->pos1.y + ssMonSummaryScreen->field_CB4->pos2.y + 4;
}
...
static u16 sub_80CC124(u16 species)
{
    ...
    for (i = 0; i < 40; i++)
    {
        if (ssMonSummaryScreen->field_B58[i] == species)
            break;
    }
    ...
    if (i == 40)
    {
        for (i = 0; i < 40; i++)
        {
            if (ssMonSummaryScreen->field_B58[i] == 0)
                break;
        }
        if (i == 40)
            return 0xFFFF;
    ...
    ssMonSummaryScreen->field_B58[i] = species;
    ssMonSummaryScreen->field_B08[i]++;
    var = 16 * i;
...
static void sub_80CC1E0(u16 species)
{
    ...
    for (i = 0; i < 40; i++)
    {
        if (ssMonSummaryScreen->field_B58[i] == species)
        {
            if (--ssMonSummaryScreen->field_B08[i] == 0)
                ssMonSummaryScreen->field_B58[i] = 0;
            break;
        }
    }
...
static void sub_80CC370(u8 taskId)
{
    ...
    {
    case 0:
        ssMonSummaryScreen->field_2D2 = 0;
        ssMonSummaryScreen->bg2_X = 0;
        task->data[1] = RequestDma3Fill(0, ssMonSummaryScreen->field_4AC4, 0x1000, 1);
        break;
    case 1:
        if (CheckForSpaceForDma3Request(task->data[1]) == -1)
            return;

        SetBgTilemapBuffer(2, ssMonSummaryScreen->field_4AC4);
        ShowBg(2);
        break;
    case 2:
...
static void SetUpScrollToBox(u8 boxId)
{
    ...
    ssMonSummaryScreen->field_2CE = (direction > 0) ? 6 : -6;
    ssMonSummaryScreen->field_2D3 = (direction > 0) ? 1 : 2;
    ssMonSummaryScreen->field_2D0 = 32;
    ssMonSummaryScreen->field_2D4 = boxId;
    ssMonSummaryScreen->field_2D6 = (direction <= 0) ? 5 : 0;
    ssMonSummaryScreen->field_2D8 = direction;
    ssMonSummaryScreen->field_2DA = (direction > 0) ? 264 : 56;
    ssMonSummaryScreen->field_2DC = (direction <= 0) ? 5 : 0;
    ssMonSummaryScreen->field_2DE = 0;
    ssMonSummaryScreen->field_2E0 = 2;
    ssMonSummaryScreen->field_A64 = boxId;
    ssMonSummaryScreen->field_A65 = direction;
    ssMonSummaryScreen->field_A63 = 0;
}
...
static bool8 ScrollToBox(void)
{
    ...
    switch (ssMonSummaryScreen->field_A63)
    {
    case 0:
        LoadWallpaperGfx(ssMonSummaryScreen->field_A64, ssMonSummaryScreen->field_A65);
        ssMonSummaryScreen->field_A63++;
    case 1:
        if (!WaitForWallpaperGfxLoad())
            return TRUE;

        sub_80CB4CC(ssMonSummaryScreen->field_A64, ssMonSummaryScreen->field_A65);
        sub_80CCCFC(ssMonSummaryScreen->field_A64, ssMonSummaryScreen->field_A65);
        sub_80CD0B8(ssMonSummaryScreen->field_A65);
        break;
    case 2:
        var = sub_80CB584();
        if (ssMonSummaryScreen->field_2D0 != 0)
        {
            ssMonSummaryScreen->bg2_X += ssMonSummaryScreen->field_2CE;
            if (--ssMonSummaryScreen->field_2D0 != 0)
                return TRUE;
            sub_80CCEE0();
            sub_80CD158();
...
static void SetWallpaperForCurrentBox(u8 wallpaperId)
{
    ...
    ssMonSummaryScreen->wallpaperChangeState = 0;
}

static bool8 DoWallpaperGfxChange(void)
{
    switch (ssMonSummaryScreen->wallpaperChangeState)
    {
    case 0:
        BeginNormalPaletteFade(ssMonSummaryScreen->field_738, 1, 0, 16, RGB_WHITEALPHA);
        ssMonSummaryScreen->wallpaperChangeState++;
        break;
    case 1:
        if (!UpdatePaletteFade())
        {
    ...
    case 2:
        ...
        {
            sub_80CCF9C();
            BeginNormalPaletteFade(ssMonSummaryScreen->field_738, 1, 16, 0, RGB_WHITEALPHA);
            ssMonSummaryScreen->wallpaperChangeState++;
        }
        break;
    case 3:
...
static void LoadWallpaperGfx(u8 boxId, s8 direction)
{
    ...
    ssMonSummaryScreen->field_6F9 = 0;
    ssMonSummaryScreen->field_6FA = boxId;
    ssMonSummaryScreen->field_6FB = direction;
    if (ssMonSummaryScreen->field_6FB != 0)
    {
        ssMonSummaryScreen->field_2D2 = (ssMonSummaryScreen->field_2D2 == 0);
        sub_80CCAE0(ssMonSummaryScreen->field_4AC4);
    }

    wallpaperId = GetBoxWallpaper(ssMonSummaryScreen->field_6FA);
    if (wallpaperId != WALLPAPER_FRIENDS)
    {
        wallpaperGfx = &gWallpaperTable[wallpaperId];
        LZ77UnCompWram(wallpaperGfx->tileMap, ssMonSummaryScreen->field_792);
        sub_80CCA3C(ssMonSummaryScreen->field_792, ssMonSummaryScreen->field_6FB, ssMonSummaryScreen->field_2D2);

        if (ssMonSummaryScreen->field_6FB != 0)
            LoadPalette(wallpaperGfx->palettes, (ssMonSummaryScreen->field_2D2 * 32) + 0x40, 0x40);
        else
            CpuCopy16(wallpaperGfx->palettes, &gPlttBufferUnfaded[(ssMonSummaryScreen->field_2D2 * 32) + 0x40], 0x40);

        ssMonSummaryScreen->wallpaperTiles = malloc_and_decompress(wallpaperGfx->tiles, &size1);
        LoadBgTiles(2, ssMonSummaryScreen->wallpaperTiles, size1, ssMonSummaryScreen->field_2D2 << 8);
    }
    else
    {
        ...
        CpuCopy16(wallpaperGfx->palettes, ssMonSummaryScreen->field_792, 0x40);
        CpuCopy16(GetWaldaWallpaperColorsPtr(), &ssMonSummaryScreen->field_792[1], 4);
        CpuCopy16(GetWaldaWallpaperColorsPtr(), &ssMonSummaryScreen->field_792[17], 4);

        if (ssMonSummaryScreen->field_6FB != 0)
            LoadPalette(ssMonSummaryScreen->field_792, (ssMonSummaryScreen->field_2D2 * 32) + 0x40, 0x40);
        else
            CpuCopy16(ssMonSummaryScreen->field_792, &gPlttBufferUnfaded[(ssMonSummaryScreen->field_2D2 * 32) + 0x40], 0x40);

        ssMonSummaryScreen->wallpaperTiles = malloc_and_decompress(wallpaperGfx->tiles, &size1);
        iconGfx = malloc_and_decompress(gFriendsIcons[GetWaldaWallpaperIconId()], &size2);
        CpuCopy32(iconGfx, ssMonSummaryScreen->wallpaperTiles + 0x800, size2);
        Free(iconGfx);
        LoadBgTiles(2, ssMonSummaryScreen->wallpaperTiles, size1, ssMonSummaryScreen->field_2D2 << 8);
    }
...
static bool32 WaitForWallpaperGfxLoad(void)
{
    ...
    if (ssMonSummaryScreen->wallpaperTiles != NULL)
    {
        Free(ssMonSummaryScreen->wallpaperTiles);
        ssMonSummaryScreen->wallpaperTiles = NULL;
    }
    return TRUE;
}
...
{
    s16 var = (arg2 * 2) + 3;
    s16 x = ((ssMonSummaryScreen->bg2_X / 8 + 10) + (direction * 24)) & 0x3F;
...
static void sub_80CCAE0(void *arg0)
{
    u16 i;
    u16 *dest = arg0;
    s16 r3 = ((ssMonSummaryScreen->bg2_X / 8) + 30) & 0x3F;
...
static void sub_80CCB50(u8 boxId)
{
    ...
    struct SpriteSheet spriteSheet = {ssMonSummaryScreen->field_2F8, 0x200, TAG_TILE_3};
    struct SpritePalette palettes[] = {
        {ssMonSummaryScreen->field_6FC, TAG_PAL_DAC9},
        {}
    };

    u16 wallpaperId = GetBoxWallpaper(boxId);

    ssMonSummaryScreen->field_6FC[14] = gUnknown_08577574[wallpaperId][0];
    ssMonSummaryScreen->field_6FC[15] = gUnknown_08577574[wallpaperId][1];
    LoadSpritePalettes(palettes);
    ssMonSummaryScreen->field_738 = 0x3f0;

    tagIndex = IndexOfSpritePaletteTag(TAG_PAL_DAC9);
    ssMonSummaryScreen->field_71C = 0x10e + 16 * tagIndex;
    ssMonSummaryScreen->field_738 |= 0x10000 << tagIndex;

    tagIndex = IndexOfSpritePaletteTag(TAG_PAL_DAC9);
    ssMonSummaryScreen->field_71E = 0x10e + 16 * tagIndex;
    ssMonSummaryScreen->field_738 |= 0x10000 << tagIndex;

    StringCopyPadded(ssMonSummaryScreen->field_21B8, GetBoxNamePtr(boxId), 0, 8);
    DrawTextWindowAndBufferTiles(ssMonSummaryScreen->field_21B8, ssMonSummaryScreen->field_2F8, 0, 0, 2);
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        ssMonSummaryScreen->field_720[i] = &gSprites[spriteId];
        StartSpriteAnim(ssMonSummaryScreen->field_720[i], i);
    }
    ssMonSummaryScreen->field_6F8 = 0;
}
...
static void sub_80CCCFC(u8 boxId, s8 direction)
{
    ...
    ssMonSummaryScreen->field_6F8 = (ssMonSummaryScreen->field_6F8 == 0);
    if (ssMonSummaryScreen->field_6F8 == 0)
    {
        spriteSheet.tag = TAG_TILE_3;
        r8 = ssMonSummaryScreen->field_71C;
    }
    else
    {
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        ssMonSummaryScreen->field_728[i] = &gSprites[spriteId];
        ssMonSummaryScreen->field_728[i]->data[0] = (-direction) * 6;
        ssMonSummaryScreen->field_728[i]->data[1] = i * 32 + x;
        ssMonSummaryScreen->field_728[i]->data[2] = 0;
        ssMonSummaryScreen->field_728[i]->callback = sub_80CCF30;
        StartSpriteAnim(ssMonSummaryScreen->field_728[i], i);

        ssMonSummaryScreen->field_720[i]->data[0] = (-direction) * 6;
        ssMonSummaryScreen->field_720[i]->data[1] = 1;
        ssMonSummaryScreen->field_720[i]->callback = sub_80CCF64;
    }
}
...
static void sub_80CCEE0(void)
{
    ...
    ssMonSummaryScreen->field_720[0] = ssMonSummaryScreen->field_728[0];
    ssMonSummaryScreen->field_720[1] = ssMonSummaryScreen->field_728[1];
}
...
static void sub_80CCF9C(void)
{
    ...
    if (ssMonSummaryScreen->field_6F8 == 0)
        CpuCopy16(gUnknown_08577574[wallpaperId], gPlttBufferUnfaded + ssMonSummaryScreen->field_71C, 4);
    else
        CpuCopy16(gUnknown_08577574[wallpaperId], gPlttBufferUnfaded + ssMonSummaryScreen->field_71E, 4);
}
...
static void sub_80CD02C(void)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        if (spriteId != MAX_SPRITES)
        {
            ...
            ssMonSummaryScreen->field_730[i] = sprite;
        }
    }
    if (IsCursorOnBox())
        sub_80CD1A8(TRUE);
...
static void sub_80CD0B8(s8 direction)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ssMonSummaryScreen->field_730[i]->pos2.x = 0;
        ssMonSummaryScreen->field_730[i]->data[0] = 2;
    }
    if (direction < 0)
    {
        ssMonSummaryScreen->field_730[0]->data[1] = 29;
        ssMonSummaryScreen->field_730[1]->data[1] = 5;
        ssMonSummaryScreen->field_730[0]->data[2] = 0x48;
        ssMonSummaryScreen->field_730[1]->data[2] = 0x48;
    }
    else
    {
        ssMonSummaryScreen->field_730[0]->data[1] = 5;
        ssMonSummaryScreen->field_730[1]->data[1] = 29;
        ssMonSummaryScreen->field_730[0]->data[2] = 0xF8;
        ssMonSummaryScreen->field_730[1]->data[2] = 0xF8;
    }
    ssMonSummaryScreen->field_730[0]->data[7] = 0;
    ssMonSummaryScreen->field_730[1]->data[7] = 1;
}
...
static void sub_80CD158(void)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ssMonSummaryScreen->field_730[i]->pos1.x = 0x88 * i + 0x5c;
        ssMonSummaryScreen->field_730[i]->pos2.x = 0;
        ssMonSummaryScreen->field_730[i]->invisible = FALSE;
    }
    sub_80CD1A8(TRUE);
}
...
static void sub_80CD1A8(bool8 a0)
{
    ...
    if (a0)
    {
        for (i = 0; i < 2; i++)
        {
            ssMonSummaryScreen->field_730[i]->data[0] = 1;
            ssMonSummaryScreen->field_730[i]->data[1] = 0;
            ssMonSummaryScreen->field_730[i]->data[2] = 0;
            ssMonSummaryScreen->field_730[i]->data[4] = 0;
        }
    }
    else
    {
        for (i = 0; i < 2; i++)
        {
            ssMonSummaryScreen->field_730[i]->data[0] = 0;
        }
    }
}
...
static void sub_80CD210(struct Sprite *sprite)
{
    ...
    case 2:
        ...
        break;
    case 3:
        sprite->pos1.x -= ssMonSummaryScreen->field_2CE;
...
static void sub_80CD36C(void)
{
    if (ssMonSummaryScreen->boxOption != BOX_OPTION_DEPOSIT)
    ...
    sub_80CDC0C();
    sub_80CFC14();
    ssMonSummaryScreen->field_CD6 = 1;
    ssMonSummaryScreen->inBoxMovingMode = 0;
    sub_80CEB40();
}
...
static void sub_80CD3EC(void)
{
    ...
    if (sIsMonBeingMoved)
    {
        ssMonSummaryScreen->movingMon = gUnknown_02039D14;
        CreateMovingMonIcon();
    }
}
...
static bool8 sub_80CD554(void)
{
    ...
    if (ssMonSummaryScreen->field_CD0 == 0)
    {
        if (ssMonSummaryScreen->boxOption != BOX_OPTION_MOVE_ITEMS)
            return FALSE;
        else
    ...
    else if (--ssMonSummaryScreen->field_CD0 != 0)
    {
        ssMonSummaryScreen->field_CBC += ssMonSummaryScreen->field_CC4;
        ssMonSummaryScreen->field_CC0 += ssMonSummaryScreen->field_CC8;
        ssMonSummaryScreen->field_CB4->pos1.x = ssMonSummaryScreen->field_CBC >> 8;
        ssMonSummaryScreen->field_CB4->pos1.y = ssMonSummaryScreen->field_CC0 >> 8;
        if (ssMonSummaryScreen->field_CB4->pos1.x > 0x100)
        {
            tmp = ssMonSummaryScreen->field_CB4->pos1.x - 0x100;
            ssMonSummaryScreen->field_CB4->pos1.x = tmp + 0x40;
        }
        if (ssMonSummaryScreen->field_CB4->pos1.x < 0x40)
        {
            tmp = 0x40 - ssMonSummaryScreen->field_CB4->pos1.x;
            ssMonSummaryScreen->field_CB4->pos1.x = 0x100 - tmp;
        }
        if (ssMonSummaryScreen->field_CB4->pos1.y > 0xb0)
        {
            tmp = ssMonSummaryScreen->field_CB4->pos1.y - 0xb0;
            ssMonSummaryScreen->field_CB4->pos1.y = tmp - 0x10;
        }
        if (ssMonSummaryScreen->field_CB4->pos1.y < -0x10)
        {
            tmp = -0x10 - ssMonSummaryScreen->field_CB4->pos1.y;
            ssMonSummaryScreen->field_CB4->pos1.y = 0xb0 - tmp;
        }
        if (ssMonSummaryScreen->field_CD7 && --ssMonSummaryScreen->field_CD7 == 0)
            ssMonSummaryScreen->field_CB4->vFlip = (ssMonSummaryScreen->field_CB4->vFlip == FALSE);
    }
    else
    {
        ssMonSummaryScreen->field_CB4->pos1.x = ssMonSummaryScreen->field_CCC;
        ssMonSummaryScreen->field_CB4->pos1.y = ssMonSummaryScreen->field_CCE;
        sub_80CDA68();
    }
...
static void sub_80CD6AC(u8 newCurosrArea, u8 newCursorPosition)
{
    ...
    ssMonSummaryScreen->field_CD4 = newCurosrArea;
    ssMonSummaryScreen->field_CD5 = newCursorPosition;
    ssMonSummaryScreen->field_CCC = x;
    ssMonSummaryScreen->field_CCE = y;
}
...
static void sub_80CD70C(void)
{
    ...
    if (ssMonSummaryScreen->field_CD2 != 0 || ssMonSummaryScreen->field_CD3 != 0)
        ssMonSummaryScreen->field_CD0 = 12;
    else
        ssMonSummaryScreen->field_CD0 = 6;

    if (ssMonSummaryScreen->field_CD7)
        ssMonSummaryScreen->field_CD7 = ssMonSummaryScreen->field_CD0 >> 1;

    switch (ssMonSummaryScreen->field_CD2)
    {
        default:
            r7 = ssMonSummaryScreen->field_CCE - ssMonSummaryScreen->field_CB4->pos1.y;
            break;
        case -1:
            r7 = ssMonSummaryScreen->field_CCE - 0xc0 - ssMonSummaryScreen->field_CB4->pos1.y;
            break;
        case 1:
            r7 = ssMonSummaryScreen->field_CCE + 0xc0 - ssMonSummaryScreen->field_CB4->pos1.y;
            break;
    ...
    switch (ssMonSummaryScreen->field_CD3)
    {
        default:
            r0 = ssMonSummaryScreen->field_CCC - ssMonSummaryScreen->field_CB4->pos1.x;
            break;
        case -1:
            r0 = ssMonSummaryScreen->field_CCC - 0xc0 - ssMonSummaryScreen->field_CB4->pos1.x;
            break;
        case 1:
            r0 = ssMonSummaryScreen->field_CCC + 0xc0 - ssMonSummaryScreen->field_CB4->pos1.x;
            break;
    ...
    r7 <<= 8;
    r0 <<= 8;
    ssMonSummaryScreen->field_CC4 = r0 / ssMonSummaryScreen->field_CD0;
    ssMonSummaryScreen->field_CC8 = r7 / ssMonSummaryScreen->field_CD0;
    ssMonSummaryScreen->field_CBC = ssMonSummaryScreen->field_CB4->pos1.x << 8;
    ssMonSummaryScreen->field_CC0 = ssMonSummaryScreen->field_CB4->pos1.y << 8;
}
...
static void sub_80CD894(u8 newCurosrArea, u8 newCursorPosition)
{
    ...
    if (ssMonSummaryScreen->boxOption != BOX_OPTION_MOVE_ITEMS)
    {
        if (ssMonSummaryScreen->inBoxMovingMode == 0 && !sIsMonBeingMoved)
            StartSpriteAnim(ssMonSummaryScreen->field_CB4, 1);
    }
    else
    {
        if (!IsActiveItemMoving())
    ...
    {
        ssMonSummaryScreen->field_CD6 = newCurosrArea;
        ssMonSummaryScreen->field_CB8->invisible = TRUE;
    }
    ...
    case CURSOR_AREA_BOX:
    case CURSOR_AREA_BUTTONS:
        ssMonSummaryScreen->field_CB4->oam.priority = 1;
        ssMonSummaryScreen->field_CB8->invisible = TRUE;
        ssMonSummaryScreen->field_CB8->oam.priority = 1;
        break;
    case CURSOR_AREA_IN_BOX:
        if (ssMonSummaryScreen->inBoxMovingMode != 0)
        {
            ssMonSummaryScreen->field_CB4->oam.priority = 0;
            ssMonSummaryScreen->field_CB8->invisible = TRUE;
        }
        else
        {
            ssMonSummaryScreen->field_CB4->oam.priority = 2;
...
static void sub_80CDA68(void)
{
    sBoxCursorArea = ssMonSummaryScreen->field_CD4;
    sBoxCursorPosition = ssMonSummaryScreen->field_CD5;
    if (ssMonSummaryScreen->boxOption != BOX_OPTION_MOVE_ITEMS)
    {
        if (ssMonSummaryScreen->inBoxMovingMode == 0 && !sIsMonBeingMoved)
            StartSpriteAnim(ssMonSummaryScreen->field_CB4, 0);
    }
    else
    {
        if (!IsActiveItemMoving())
    ...
    case CURSOR_AREA_BOX:
        ...
        break;
    case CURSOR_AREA_IN_PARTY:
        ssMonSummaryScreen->field_CB8->subpriority = 13;
        SetMovingMonPriority(1);
        break;
    case CURSOR_AREA_IN_BOX:
        if (ssMonSummaryScreen->inBoxMovingMode == 0)
        {
            ssMonSummaryScreen->field_CB4->oam.priority = 1;
            ssMonSummaryScreen->field_CB8->oam.priority = 2;
            ssMonSummaryScreen->field_CB8->subpriority = 21;
            ssMonSummaryScreen->field_CB8->invisible = FALSE;
            SetMovingMonPriority(2);
        }
        break;
    }
...
static void sub_80CDBA0(void)
{
    ...
    if (ssMonSummaryScreen->field_CB4->vFlip)
        ssMonSummaryScreen->field_CD7 = 1;
...
static void InitMonPlaceChange(u8 a0)
{
    ...
    ssMonSummaryScreen->monPlaceChangeFunc = placeChangeFuncs[a0];
    ssMonSummaryScreen->monPlaceChangeState = 0;
}

static void sub_80CDC64(bool8 arg0)
{
    if (!arg0)
        ssMonSummaryScreen->monPlaceChangeFunc = sub_80CDEB4;
    else
        ssMonSummaryScreen->monPlaceChangeFunc = sub_80CDEC4;
...
static bool8 DoMonPlaceChange(void)
{
    return ssMonSummaryScreen->monPlaceChangeFunc();
}

static bool8 MonPlaceChange_Move(void)
{
    switch (ssMonSummaryScreen->monPlaceChangeState)
    {
    case 0:
        if (sIsMonBeingMoved)
            return FALSE;
        StartSpriteAnim(ssMonSummaryScreen->field_CB4, 2);
        ssMonSummaryScreen->monPlaceChangeState++;
        break;
    case 1:
        if (!sub_80CDED4())
        {
            StartSpriteAnim(ssMonSummaryScreen->field_CB4, 3);
            MoveMon();
            ssMonSummaryScreen->monPlaceChangeState++;
        }
        break;
...
static bool8 MonPlaceChange_Shift(void)
{
    ...
    case 0:
        ...
        {
        case CURSOR_AREA_IN_PARTY:
            ssMonSummaryScreen->field_D91 = TOTAL_BOXES_COUNT;
            break;
        case CURSOR_AREA_IN_BOX:
            ssMonSummaryScreen->field_D91 = StorageGetCurrentBox();
            break;
        default:
            return FALSE;
        }
        StartSpriteAnim(ssMonSummaryScreen->field_CB4, 2);
        sub_80CBD5C(ssMonSummaryScreen->field_D91, sBoxCursorPosition);
        ssMonSummaryScreen->monPlaceChangeState++;
        break;
    case 1:
        if (!sub_80CBDC4())
        {
            StartSpriteAnim(ssMonSummaryScreen->field_CB4, 3);
            SetShiftedMonData(ssMonSummaryScreen->field_D91, sBoxCursorPosition);
            ssMonSummaryScreen->monPlaceChangeState++;
        }
        break;
    case 2:
...
static bool8 sub_80CDED4(void)
{
    switch (ssMonSummaryScreen->field_CB4->pos2.y)
    {
    default:
        ssMonSummaryScreen->field_CB4->pos2.y++;
        break;
    case 0:
        ssMonSummaryScreen->field_CB4->pos2.y++;
        break;
...
static void SetMovedMonData(u8 boxId, u8 position)
{
    if (boxId == TOTAL_BOXES_COUNT)
        ssMonSummaryScreen->movingMon = gPlayerParty[sBoxCursorPosition];
    else
        BoxMonAtToMon(boxId, position, &ssMonSummaryScreen->movingMon);
...
static void SetPlacedMonData(u8 boxId, u8 position)
{
    if (boxId == TOTAL_BOXES_COUNT)
    {
        gPlayerParty[position] = ssMonSummaryScreen->movingMon;
    }
    else
    {
        BoxMonRestorePP(&ssMonSummaryScreen->movingMon.box);
        SetBoxMonAt(boxId, position, &ssMonSummaryScreen->movingMon.box);
    }
}
...
{
    if (boxId == TOTAL_BOXES_COUNT)
        ssMonSummaryScreen->field_2108 = gPlayerParty[position];
    else
        BoxMonAtToMon(boxId, position, &ssMonSummaryScreen->field_2108);

    SetPlacedMonData(boxId, position);
    ssMonSummaryScreen->movingMon = ssMonSummaryScreen->field_2108;
    SetCursorMonData(&ssMonSummaryScreen->movingMon, MODE_PARTY);
    sMovingMonOrigBoxId = boxId;
    sMovingMonOrigBoxPos = position;
}
...
static void sub_80CE250(void)
{
    ...
    StringCopy(ssMonSummaryScreen->field_21E0, ssMonSummaryScreen->cursorMonNick);
}
...
static void InitCanRelaseMonVars(void)
{
    ...
    {
        ssMonSummaryScreen->field_216D = 1;
        ssMonSummaryScreen->field_216C = 0;
        return;
    }

    if (sIsMonBeingMoved)
    {
        ssMonSummaryScreen->field_2108 = ssMonSummaryScreen->movingMon;
        ssMonSummaryScreen->field_2170 = -1;
        ssMonSummaryScreen->field_2171 = -1;
    }
    else
    {
        if (sBoxCursorArea == CURSOR_AREA_IN_PARTY)
        {
            ssMonSummaryScreen->field_2108 = gPlayerParty[sBoxCursorPosition];
            ssMonSummaryScreen->field_2170 = TOTAL_BOXES_COUNT;
        }
        else
        {
            BoxMonAtToMon(StorageGetCurrentBox(), sBoxCursorPosition, &ssMonSummaryScreen->field_2108);
            ssMonSummaryScreen->field_2170 = StorageGetCurrentBox();
        }
        ssMonSummaryScreen->field_2171 = sBoxCursorPosition;
    }

    sub_80CE350(ssMonSummaryScreen->field_2176);
    ssMonSummaryScreen->field_2174 = GetMonData(&ssMonSummaryScreen->field_2108, MON_DATA_KNOWN_MOVES, (u8*)ssMonSummaryScreen->field_2176);
    if (ssMonSummaryScreen->field_2174 != 0)
    {
        ssMonSummaryScreen->field_216D = 0;
    }
    else
    {
        ssMonSummaryScreen->field_216D = 1;
        ssMonSummaryScreen->field_216C = 1;
    }

    ssMonSummaryScreen->field_2172 = 0;
}
...
static s8 RunCanReleaseMon(void)
{
    ...
    if (ssMonSummaryScreen->field_216D)
        return ssMonSummaryScreen->field_216C;

    switch (ssMonSummaryScreen->field_2172)
    {
    case 0:
        for (i = 0; i < PARTY_SIZE; i++)
        {
            if (ssMonSummaryScreen->field_2170 != TOTAL_BOXES_COUNT || ssMonSummaryScreen->field_2171 != i)
            {
                knownMoves = GetMonData(gPlayerParty + i, MON_DATA_KNOWN_MOVES, (u8*)ssMonSummaryScreen->field_2176);
                ssMonSummaryScreen->field_2174 &= ~(knownMoves);
            }
        }
        if (ssMonSummaryScreen->field_2174 == 0)
        {
        ...
        {
            ssMonSummaryScreen->field_216E = 0;
            ssMonSummaryScreen->field_216F = 0;
            ssMonSummaryScreen->field_2172++;
        }
        break;
    case 1:
        for (i = 0; i < IN_BOX_COUNT; i++)
        {
            knownMoves = GetAndCopyBoxMonDataAt(ssMonSummaryScreen->field_216E, ssMonSummaryScreen->field_216F, MON_DATA_KNOWN_MOVES, (u8*)ssMonSummaryScreen->field_2176);
            if (knownMoves != 0
                && !(ssMonSummaryScreen->field_2170 == ssMonSummaryScreen->field_216E && ssMonSummaryScreen->field_2171 == ssMonSummaryScreen->field_216F))
            {
                ssMonSummaryScreen->field_2174 &= ~(knownMoves);
                if (ssMonSummaryScreen->field_2174 == 0)
                {
            ...
            if (++ssMonSummaryScreen->field_216F >= IN_BOX_COUNT)
            {
                ssMonSummaryScreen->field_216F = 0;
                if (++ssMonSummaryScreen->field_216E >= TOTAL_BOXES_COUNT)
                {
                    ssMonSummaryScreen->field_216D = 1;
                    ssMonSummaryScreen->field_216C = 0;
                }
...
{
    if (sIsMonBeingMoved)
        gUnknown_02039D14 = ssMonSummaryScreen->movingMon;
}
...
static void sub_80CE790(void)
{
    if (sIsMonBeingMoved)
    {
        if (sMovingMonOrigBoxId == TOTAL_BOXES_COUNT)
            ...
            ssMonSummaryScreen->movingMon.box = gUnknown_02039D14.box;
    }
}
...
static void sub_80CE7E8(void)
{
    ...
    {
        sub_80CE760();
        ssMonSummaryScreen->field_218C.mon = &gUnknown_02039D14;
        ssMonSummaryScreen->field_2187 = 0;
        ssMonSummaryScreen->field_2186 = 0;
        ssMonSummaryScreen->field_2188 = 0;
    }
    else if (sBoxCursorArea == CURSOR_AREA_IN_PARTY)
    {
        ssMonSummaryScreen->field_218C.mon = gPlayerParty;
        ssMonSummaryScreen->field_2187 = sBoxCursorPosition;
        ssMonSummaryScreen->field_2186 = CountPartyMons() - 1;
        ssMonSummaryScreen->field_2188 = 0;
    }
    else
    {
        ssMonSummaryScreen->field_218C.box = GetBoxedMonPtr(StorageGetCurrentBox(), 0);
        ssMonSummaryScreen->field_2187 = sBoxCursorPosition;
        ssMonSummaryScreen->field_2186 = IN_BOX_COUNT - 1;
        ssMonSummaryScreen->field_2188 = 2;
    }
}
...
static void SetMonMarkings(u8 markings)
{
    ssMonSummaryScreen->cursorMonMarkings = markings;
    if (sIsMonBeingMoved)
    {
        SetMonData(&ssMonSummaryScreen->movingMon, MON_DATA_MARKINGS, &markings);
    }
    else
    {
        if (sBoxCursorArea == CURSOR_AREA_IN_PARTY)
...
static bool8 CanShiftMon(void)
{
    if (sIsMonBeingMoved)
    {
        ...
        {
            if (ssMonSummaryScreen->cursorMonIsEgg || GetMonData(&ssMonSummaryScreen->movingMon, MON_DATA_HP) == 0)
                return FALSE;
        }
        return TRUE;
    }
...
static void sub_80CEB40(void)
{
    ssMonSummaryScreen->setMosaic = (sIsMonBeingMoved == FALSE);
    if (!sIsMonBeingMoved)
    {
        switch (sBoxCursorArea)
        {
...
static void SetCursorMonData(void *pokemon, u8 mode)
{
    ...
    ssMonSummaryScreen->cursorMonItem = 0;
    gender = MON_MALE;
    ...
    if (mode == MODE_PARTY)
    {
        ...
        ssMonSummaryScreen->cursorMonSpecies = GetMonData(mon, MON_DATA_SPECIES2);
        if (ssMonSummaryScreen->cursorMonSpecies != SPECIES_NONE)
        {
            sanityIsBagEgg = GetMonData(mon, MON_DATA_SANITY_IS_BAD_EGG);
            if (sanityIsBagEgg)
                ssMonSummaryScreen->cursorMonIsEgg = TRUE;
            else
                ssMonSummaryScreen->cursorMonIsEgg = GetMonData(mon, MON_DATA_IS_EGG);

            GetMonData(mon, MON_DATA_NICKNAME, ssMonSummaryScreen->cursorMonNick);
            StringGetEnd10(ssMonSummaryScreen->cursorMonNick);
            ssMonSummaryScreen->cursorMonLevel = GetMonData(mon, MON_DATA_LEVEL);
            ssMonSummaryScreen->cursorMonMarkings = GetMonData(mon, MON_DATA_MARKINGS);
            ssMonSummaryScreen->cursorMonPersonality = GetMonData(mon, MON_DATA_PERSONALITY);
            ssMonSummaryScreen->cursorMonPalette = GetMonFrontSpritePal(mon);
            gender = GetMonGender(mon);
            ssMonSummaryScreen->cursorMonItem = GetMonData(mon, MON_DATA_HELD_ITEM);
        }
    }
    else if (mode == MODE_BOX)
    {
        ...
        ssMonSummaryScreen->cursorMonSpecies = GetBoxMonData(pokemon, MON_DATA_SPECIES2);
        if (ssMonSummaryScreen->cursorMonSpecies != SPECIES_NONE)
        {
            ...
            if (sanityIsBagEgg)
                ...
                ssMonSummaryScreen->cursorMonIsEgg = GetBoxMonData(boxMon, MON_DATA_IS_EGG);


            GetBoxMonData(boxMon, MON_DATA_NICKNAME, ssMonSummaryScreen->cursorMonNick);
            StringGetEnd10(ssMonSummaryScreen->cursorMonNick);
            ssMonSummaryScreen->cursorMonLevel = GetLevelFromBoxMonExp(boxMon);
            ssMonSummaryScreen->cursorMonMarkings = GetBoxMonData(boxMon, MON_DATA_MARKINGS);
            ssMonSummaryScreen->cursorMonPersonality = GetBoxMonData(boxMon, MON_DATA_PERSONALITY);
            ssMonSummaryScreen->cursorMonPalette = GetMonSpritePalFromSpeciesAndPersonality(ssMonSummaryScreen->cursorMonSpecies, otId, ssMonSummaryScreen->cursorMonPersonality);
            gender = GetGenderFromSpeciesAndPersonality(ssMonSummaryScreen->cursorMonSpecies, ssMonSummaryScreen->cursorMonPersonality);
            ssMonSummaryScreen->cursorMonItem = GetBoxMonData(boxMon, MON_DATA_HELD_ITEM);
        }
    }
    else
    {
        ssMonSummaryScreen->cursorMonSpecies = SPECIES_NONE;
        ssMonSummaryScreen->cursorMonItem = 0;
    }

    if (ssMonSummaryScreen->cursorMonSpecies == SPECIES_NONE)
    {
        StringFill(ssMonSummaryScreen->cursorMonNick, CHAR_SPACE, 5);
        StringFill(ssMonSummaryScreen->cursorMonNickText, CHAR_SPACE, 8);
        StringFill(ssMonSummaryScreen->cursorMonSpeciesName, CHAR_SPACE, 8);
        StringFill(ssMonSummaryScreen->cursorMonGenderLvlText, CHAR_SPACE, 8);
        StringFill(ssMonSummaryScreen->cursorMonItemName, CHAR_SPACE, 8);
    }
    else if (ssMonSummaryScreen->cursorMonIsEgg)
    {
        if (sanityIsBagEgg)
            StringCopyPadded(ssMonSummaryScreen->cursorMonNickText, ssMonSummaryScreen->cursorMonNick, CHAR_SPACE, 5);
        else
            StringCopyPadded(ssMonSummaryScreen->cursorMonNickText, gText_EggNickname, CHAR_SPACE, 8);
    ...
    {
        if (ssMonSummaryScreen->cursorMonSpecies == SPECIES_NIDORAN_F || ssMonSummaryScreen->cursorMonSpecies == SPECIES_NIDORAN_M)
            gender = MON_GENDERLESS;

        StringCopyPadded(ssMonSummaryScreen->cursorMonNickText, ssMonSummaryScreen->cursorMonNick, CHAR_SPACE, 5);

        txtPtr = ssMonSummaryScreen->cursorMonSpeciesName;
        *(txtPtr)++ = CHAR_SLASH;
        StringCopyPadded(txtPtr, gSpeciesNames[ssMonSummaryScreen->cursorMonSpecies], CHAR_SPACE, 5);

        txtPtr = ssMonSummaryScreen->cursorMonGenderLvlText;
        ...
        *(txtPtr++) = 5;
        ...
        txtPtr = ConvertIntToDecimalStringN(txtPtr, ssMonSummaryScreen->cursorMonLevel, STR_CONV_MODE_LEFT_ALIGN, 3);
        txtPtr[0] = CHAR_SPACE;
        txtPtr[1] = EOS;

        if (ssMonSummaryScreen->cursorMonItem != 0)
            StringCopyPadded(ssMonSummaryScreen->cursorMonItemName, ItemId_GetName(ssMonSummaryScreen->cursorMonItem), CHAR_SPACE, 8);
        else
            StringFill(ssMonSummaryScreen->cursorMonItemName, CHAR_SPACE, 8);
    }
}
...
{
    switch (ssMonSummaryScreen->inBoxMovingMode)
    {
    case 0:
    default:
...
static u8 InBoxInput_Normal(void)
{
    ...
    ssMonSummaryScreen->field_CD2 = 0;
    ssMonSummaryScreen->field_CD3 = 0;
    ssMonSummaryScreen->field_CD7 = 0;
    ...
    do
    {
        ...
        else if (gMain.newAndRepeatedKeys & DPAD_DOWN)
        {
            ...
            if (cursorPosition >= IN_BOX_COUNT)
            {
                ...
                cursorPosition /= 3;
                ssMonSummaryScreen->field_CD2 = 1;
                ssMonSummaryScreen->field_CD7 = 1;
            }
            break;
        }
        else if (gMain.newAndRepeatedKeys & DPAD_LEFT)
        {
            ...
            {
                ssMonSummaryScreen->field_CD3 = -1;
        ...
        if ((gMain.newKeys & A_BUTTON) && sub_80CFA5C())
        {
            ...
            if (ssMonSummaryScreen->boxOption != BOX_OPTION_MOVE_MONS || sIsMonBeingMoved == TRUE)
            {
            ...
            {
                ssMonSummaryScreen->inBoxMovingMode = 1;
                return 20;
            }
        }
...
static u8 InBoxInput_GrabbingMultiple(void)
{
    ...
    else
    {
        ...
        {
            sIsMonBeingMoved = (ssMonSummaryScreen->cursorMonSpecies != SPECIES_NONE);
            ssMonSummaryScreen->inBoxMovingMode = 2;
...
static u8 HandleInput_InParty(void)
{
    ...
    do
    {
        ...
        {
            retVal = 1;
            ssMonSummaryScreen->field_CD6 = sBoxCursorPosition;
            cursorPosition = 0;
            break;
        }
        else if (gMain.newAndRepeatedKeys & DPAD_RIGHT)
        {
            ...
            {
                retVal = 1;
                cursorPosition = ssMonSummaryScreen->field_CD6;
            }
            else
            {
                retVal = 6;
...
static bool8 sub_80CFA84(void)
{
    ...
    switch (ssMonSummaryScreen->boxOption)
    {
...
static bool8 sub_80CFB44(void)
{
    if (ssMonSummaryScreen->cursorMonSpecies == SPECIES_EGG)
        return FALSE;
    ...
    else
    {
        ...
        {
            if (ItemIsMail(ssMonSummaryScreen->cursorMonItem) == TRUE)
                return FALSE;
...
{
    sprite->pos1.x = ssMonSummaryScreen->field_CB4->pos1.x;
    sprite->pos1.y = ssMonSummaryScreen->field_CB4->pos1.y + 20;
}
...
static void sub_80CFC14(void)
{
    ...
    ssMonSummaryScreen->field_CD8[0] = IndexOfSpritePaletteTag(TAG_PAL_WAVEFORM);
    ssMonSummaryScreen->field_CD8[1] = IndexOfSpritePaletteTag(TAG_PAL_DAC7);

    sub_80CD444(sBoxCursorArea, sBoxCursorPosition, &x, &y);
    spriteId = CreateSprite(&gSpriteTemplate_857BA50, x, y, 6);
    if (spriteId != MAX_SPRITES)
    {
        ssMonSummaryScreen->field_CB4 = &gSprites[spriteId];
        ssMonSummaryScreen->field_CB4->oam.paletteNum = ssMonSummaryScreen->field_CD8[sCanOnlyMove];
        ssMonSummaryScreen->field_CB4->oam.priority = 1;
        if (sIsMonBeingMoved)
            StartSpriteAnim(ssMonSummaryScreen->field_CB4, 3);
    }
    ...
    {
        ssMonSummaryScreen->field_CB4 = NULL;
    }
    ...
    if (spriteId != MAX_SPRITES)
    {
        ssMonSummaryScreen->field_CB8 = &gSprites[spriteId];
        ssMonSummaryScreen->field_CB8->oam.priority = priority;
        if (sBoxCursorArea)
            ssMonSummaryScreen->field_CB8->invisible = 1;
    }
    else
    {
        ssMonSummaryScreen->field_CB8 = NULL;
    }
}
...
static void sub_80CFE54(u8 animNum)
{
    StartSpriteAnim(ssMonSummaryScreen->field_CB4, animNum);
}
...
static void InitMenu(void)
{
    ssMonSummaryScreen->menuItemsCount = 0;
    ssMonSummaryScreen->menuWidth = 0;
    ssMonSummaryScreen->menuWindow.bg = 0;
    ssMonSummaryScreen->menuWindow.paletteNum = 15;
    ssMonSummaryScreen->menuWindow.baseBlock = 92;
}
...
static void SetMenuText(u8 textId)
{
    if (ssMonSummaryScreen->menuItemsCount < 7)
    {
        u8 len;
        struct StorageMenu *menu = &ssMonSummaryScreen->menuItems[ssMonSummaryScreen->menuItemsCount];

        menu->text = gUnknown_0857BA80[textId];
        menu->textId = textId;
        len = StringLength(menu->text);
        if (len > ssMonSummaryScreen->menuWidth)
            ssMonSummaryScreen->menuWidth = len;

        ssMonSummaryScreen->menuItemsCount++;
    }
}

static s8 sub_80CFF98(u8 arg0)
{
    if (arg0 >= ssMonSummaryScreen->menuItemsCount)
        return -1;
    else
        return ssMonSummaryScreen->menuItems[arg0].textId;
}

static void AddMenu(void)
{
    ssMonSummaryScreen->menuWindow.width = ssMonSummaryScreen->menuWidth + 2;
    ssMonSummaryScreen->menuWindow.height = 2 * ssMonSummaryScreen->menuItemsCount;
    ssMonSummaryScreen->menuWindow.tilemapLeft = 29 - ssMonSummaryScreen->menuWindow.width;
    ssMonSummaryScreen->menuWindow.tilemapTop = 15 - ssMonSummaryScreen->menuWindow.height;
    ssMonSummaryScreen->field_CB0 = AddWindow(&ssMonSummaryScreen->menuWindow);
    ClearWindowTilemap(ssMonSummaryScreen->field_CB0);
    DrawStdFrameWithCustomTileAndPalette(ssMonSummaryScreen->field_CB0, FALSE, 11, 14);
    PrintMenuTable(ssMonSummaryScreen->field_CB0, ssMonSummaryScreen->menuItemsCount, (void*)ssMonSummaryScreen->menuItems);
    InitMenuInUpperLeftCornerPlaySoundWhenAPressed(ssMonSummaryScreen->field_CB0, ssMonSummaryScreen->menuItemsCount, 0);
    schedule_bg_copy_tilemap_to_vram(0);
    ssMonSummaryScreen->field_CAE = 0;
}
...
static s16 sub_80D00AC(void)
{
    ...
    if (textId >= 0)
        textId = ssMonSummaryScreen->menuItems[textId].textId;
...
static void sub_80D013C(void)
{
    ClearStdWindowAndFrameToTransparent(ssMonSummaryScreen->field_CB0, TRUE);
    RemoveWindow(ssMonSummaryScreen->field_CB0);
}
...
static bool8 sub_80D0164(void)
{
    ...
    {
        ssMonSummaryScreen->field_2200 = AddWindow8Bit(&gUnknown_0857BB1C);
        if (ssMonSummaryScreen->field_2200 != 0xFF)
        {
            FillWindowPixelBuffer(ssMonSummaryScreen->field_2200, PIXEL_FILL(0));
            return TRUE;
        }
    }
...
static bool8 sub_80D024C(void)
{
    ...
    case 1:
        ...
        FillWindowPixelBuffer8Bit(ssMonSummaryScreen->field_2200, PIXEL_FILL(0));
        sub_80D07B0(sMoveMonsPtr->fromRow, sMoveMonsPtr->fromColumn);
        SetBgAttribute(0, BG_ATTR_PALETTEMODE, 1);
        PutWindowTilemap(ssMonSummaryScreen->field_2200);
        CopyWindowToVram8Bit(ssMonSummaryScreen->field_2200, 3);
...
static bool8 sub_80D03B0(void)
{
    ...
    case 0:
        if (!sub_80CD554())
        {
            ...
            CopyWindowToVram8Bit(ssMonSummaryScreen->field_2200, 2);
            sMoveMonsPtr->state++;
        }
        break;
    case 1:
...
static void sub_80D07B0(u8 arg0, u8 arg1)
{
    ...
    if (species != SPECIES_NONE)
    {
        ...
        BlitBitmapRectToWindow4BitTo8Bit(ssMonSummaryScreen->field_2200,
                                         iconGfx,
                                         0,
                                         0,
                                         32,
...
static void sub_80D0834(u8 arg0, u8 arg1)
{
    ...
    if (species != SPECIES_NONE)
    {
        FillWindowPixelRect8Bit(ssMonSummaryScreen->field_2200,
                                PIXEL_FILL(0),
                                24 * arg0,
                                24 * arg1,
                                32,
...
static void sub_80D0C60(void)
{
    ...
    if (ssMonSummaryScreen->boxOption == BOX_OPTION_MOVE_ITEMS)
    {
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            ssMonSummaryScreen->field_2204[i].tiles = GetSpriteTileStartByTag(spriteSheet.tag) * 32 + (void*)(OBJ_VRAM0);
            ssMonSummaryScreen->field_2204[i].palIndex = AllocSpritePalette(TAG_PAL_DACB + i);
            ssMonSummaryScreen->field_2204[i].palIndex *= 16;
            ssMonSummaryScreen->field_2204[i].palIndex += 0x100;
            spriteTemplate.tileTag = TAG_TILE_7 + i;
            spriteTemplate.paletteTag = TAG_PAL_DACB + i;
            spriteId = CreateSprite(&spriteTemplate, 0, 0, 11);
            ssMonSummaryScreen->field_2204[i].sprite = &gSprites[spriteId];
            ssMonSummaryScreen->field_2204[i].sprite->invisible = TRUE;
            ssMonSummaryScreen->field_2204[i].unk10 = 0;
        }
    }
    ssMonSummaryScreen->movingItem = 0;
}
...
static void Item_FromMonToMoving(u8 cursorArea, u8 cursorPos)
{
    ...
    ssMonSummaryScreen->movingItem = ssMonSummaryScreen->cursorMonItem;
}
...
static void sub_80D0F38(u16 item)
{
    ...
    sub_80D140C(id, 2, 0);
    sub_80D1740(id, TRUE);
    ssMonSummaryScreen->movingItem = item;
}
...
static void Item_SwitchMonsWithMoving(u8 cursorArea, u8 cursorPos)
{
    ...
    if (cursorArea == CURSOR_AREA_IN_BOX)
    {
        ...
        SetCurrentBoxMonData(cursorPos, MON_DATA_HELD_ITEM, &ssMonSummaryScreen->movingItem);
        ssMonSummaryScreen->movingItem = item;
    }
    else
    {
        item = GetMonData(&gPlayerParty[cursorPos], MON_DATA_HELD_ITEM);
        SetMonData(&gPlayerParty[cursorPos], MON_DATA_HELD_ITEM, &ssMonSummaryScreen->movingItem);
        ssMonSummaryScreen->movingItem = item;
    }
...
static void sub_80D11CC(void)
{
    ...
    for (i = 0; i < 3; i++)
    {
        if (ssMonSummaryScreen->field_2204[i].unk10 && ssMonSummaryScreen->field_2204[i].unk8 == 1)
            sub_80D1604(i, 7, 2, 0);
    }
}
...
static bool8 sub_80D1218(void)
{
    ...
    for (i = 0; i < 3; i++)
    {
        if (ssMonSummaryScreen->field_2204[i].unk10)
        {
            if (!ssMonSummaryScreen->field_2204[i].sprite->affineAnimEnded && ssMonSummaryScreen->field_2204[i].sprite->affineAnimBeginning)
                return TRUE;
            if (ssMonSummaryScreen->field_2204[i].sprite->callback != SpriteCallbackDummy && ssMonSummaryScreen->field_2204[i].sprite->callback != sub_80D1AD8)
                return TRUE;
        }
    }
...
static bool8 IsActiveItemMoving(void)
{
    ...
    if (ssMonSummaryScreen->boxOption == BOX_OPTION_MOVE_ITEMS)
    {
        for (i = 0; i < 3; i++)
        {
            if (ssMonSummaryScreen->field_2204[i].unk10 && ssMonSummaryScreen->field_2204[i].unk8 == 2)
                return TRUE;
        }
    }
...
static const u8 *GetMovingItemName(void)
{
    return ItemId_GetName(ssMonSummaryScreen->movingItem);
}

static u16 GetMovingItem(void)
{
    return ssMonSummaryScreen->movingItem;
}
...
static u8 sub_80D12E8(void)
{
    ...
    for (i = 0; i < 3; i++)
    {
        if (ssMonSummaryScreen->field_2204[i].unk10 == 0)
        {
            ssMonSummaryScreen->field_2204[i].unk10 = 1;
            return i;
        }
    }
...
static bool32 sub_80D1324(u8 cursorArea, u8 cursorPos)
{
    ...
    {
        if (ssMonSummaryScreen->field_2204[i].unk10
            && ssMonSummaryScreen->field_2204[i].unk8 == cursorArea
            && ssMonSummaryScreen->field_2204[i].unk9 == cursorPos)
            return TRUE;
    }
...
static u8 sub_80D13C4(struct Sprite *sprite)
{
    ...
    {
        if (ssMonSummaryScreen->field_2204[i].unk10
            && ssMonSummaryScreen->field_2204[i].sprite == sprite)
            return i;
    }
...
static void sub_80D140C(u8 id, u8 cursorArea, u8 cursorPos)
{
    ...
    case CURSOR_AREA_IN_BOX:
        ...
        ssMonSummaryScreen->field_2204[id].sprite->pos1.x = (24 * row) + 112;
        ssMonSummaryScreen->field_2204[id].sprite->pos1.y = (24 * column) + 56;
        ssMonSummaryScreen->field_2204[id].sprite->oam.priority = 2;
        break;
    case CURSOR_AREA_IN_PARTY:
        if (cursorPos == 0)
        {
            ssMonSummaryScreen->field_2204[id].sprite->pos1.x = 116;
            ssMonSummaryScreen->field_2204[id].sprite->pos1.y = 76;
        }
        else
        {
            ssMonSummaryScreen->field_2204[id].sprite->pos1.x = 164;
            ssMonSummaryScreen->field_2204[id].sprite->pos1.y = 24 * (cursorPos - 1) + 28;
        }
        ssMonSummaryScreen->field_2204[id].sprite->oam.priority = 1;
        break;
    }

    ssMonSummaryScreen->field_2204[id].unk8 = cursorArea;
    ssMonSummaryScreen->field_2204[id].unk9 = cursorPos;
}
...
static void sub_80D1524(u8 id, const u32 *itemTiles, const u32 *itemPal)
{
    ...
    CpuFastFill(0, ssMonSummaryScreen->field_42C4, 0x200);
    LZ77UnCompWram(itemTiles, ssMonSummaryScreen->field_22C4);
    for (i = 0; i < 3; i++)
        CpuFastCopy(ssMonSummaryScreen->field_22C4 + (i * 0x60), ssMonSummaryScreen->field_42C4 + (i * 0x80), 0x60);

    CpuFastCopy(ssMonSummaryScreen->field_42C4, ssMonSummaryScreen->field_2204[id].tiles, 0x200);
    LZ77UnCompWram(itemPal, ssMonSummaryScreen->field_42C4);
    LoadPalette(ssMonSummaryScreen->field_42C4, ssMonSummaryScreen->field_2204[id].palIndex, 0x20);
}
...
static void sub_80D15D4(u8 id, u8 animNum)
{
    ...
    StartSpriteAffineAnim(ssMonSummaryScreen->field_2204[id].sprite, animNum);
}
...
static void sub_80D1604(u8 id, u8 arg1, u8 arg2, u8 arg3)
{
    ...
    {
    case 0:
        ssMonSummaryScreen->field_2204[id].sprite->data[0] = id;
        ssMonSummaryScreen->field_2204[id].sprite->callback = sub_80D1A48;
        break;
    case 1:
        ssMonSummaryScreen->field_2204[id].sprite->data[0] = 0;
        ssMonSummaryScreen->field_2204[id].sprite->callback = sub_80D1A74;
        break;
    case 2:
        ssMonSummaryScreen->field_2204[id].sprite->data[0] = 0;
        ssMonSummaryScreen->field_2204[id].sprite->data[6] = arg2;
        ssMonSummaryScreen->field_2204[id].sprite->data[7] = arg3;
        ssMonSummaryScreen->field_2204[id].sprite->callback = sub_80D1B14;
        break;
    case 3:
        ssMonSummaryScreen->field_2204[id].sprite->data[0] = 0;
        ssMonSummaryScreen->field_2204[id].sprite->callback = sub_80D1B94;
        ssMonSummaryScreen->field_2204[id].sprite->data[6] = arg2;
        ssMonSummaryScreen->field_2204[id].sprite->data[7] = arg3;
        break;
    case 4:
        ...
        ssMonSummaryScreen->field_2204[id].sprite->data[6] = arg2;
        ssMonSummaryScreen->field_2204[id].sprite->data[7] = arg3;
        ssMonSummaryScreen->field_2204[id].sprite->callback = sub_80D1C30;
        break;
    case 7:
        ssMonSummaryScreen->field_2204[id].sprite->callback = sub_80D1CCC;
        break;
    }
}
...
static void sub_80D1740(u8 id, bool8 arg1)
{
    ...
    ssMonSummaryScreen->field_2204[id].unk10 = arg1;
    ssMonSummaryScreen->field_2204[id].sprite->invisible = (arg1 == FALSE);
}
...
static void PrintItemDescription(void)
{
    ...
    if (IsActiveItemMoving())
        description = ItemId_GetDescription(ssMonSummaryScreen->movingItem);
    else
        description = ItemId_GetDescription(ssMonSummaryScreen->cursorMonItem);
...
static void sub_80D1818(void)
{
    ssMonSummaryScreen->field_2236 = 0x15;
...
static bool8 sub_80D184C(void)
{
    ...
    if (ssMonSummaryScreen->field_2236 == 0)
        return FALSE;

    ssMonSummaryScreen->field_2236--;
    var = 0x15 - ssMonSummaryScreen->field_2236;
    for (i = 0; i < var; i++)
    {
        WriteSequenceToBgTilemapBuffer(0, GetBgAttribute(0, BG_ATTR_BASETILE) + 0x14 + ssMonSummaryScreen->field_2236 + i, i, 13, 1, 7, 15, 21);
    }

    sub_80D19B4(var);
    return (ssMonSummaryScreen->field_2236 != 0);
}
...
static bool8 sub_80D18E4(void)
{
    ...
    if (ssMonSummaryScreen->field_2236 == 0x16)
        return FALSE;
...
static void sub_80D1AD8(struct Sprite *sprite)
{
    sprite->pos1.x = ssMonSummaryScreen->field_CB4->pos1.x + 4;
    sprite->pos1.y = ssMonSummaryScreen->field_CB4->pos1.y + ssMonSummaryScreen->field_CB4->pos2.y + 8;
    sprite->oam.priority = ssMonSummaryScreen->field_CB4->oam.priority;
}
